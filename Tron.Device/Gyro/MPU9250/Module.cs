...
namespace Tron.Device.Gyro.MPU9250
{
    public class Module : Hardware.I2CDevice<Register>, IGyro
    {
        ...
        private void Initiailze(Ascale a, Gscale g, byte sampleRate)
        {
            if (this.ID != 0x71)
            {
            ...
            this.WriteByte(Register.SMPLRT_DIV, sampleRate);        // Use a 200 Hz rate; a rate consistent with the filter update rate 
            ...
            c = (byte)(c | (byte)g << 3);       // Set full scale range for the gyro
            ...
            // c = c & ~0xE0;		// Clear self-test bits [7:5] 
            c = (byte)(c & ~0x18);		// Clear AFS bits [4:3]
            c = (byte)(c | (byte)a << 3);		// Set full scale range for the accelerometer 
